...
describe('DataManagement', () => {
  ...
  it('imports saved teams and merges them', async () => {
      ...
      await waitFor(() => {
          expect(teamStorage.saveTeamList).toHaveBeenCalledWith(expect.arrayContaining([
              expect.objectContaining({ id: 'new-team', name: 'Imported Team' })
          ]));
      });
  });
  ...
  it('handles ID collision by generating new ID', async () => {
      ...
      // Cleanup
      if (originalRandomUUID) Object.defineProperty(crypto, 'randomUUID', { value: originalRandomUUID });
  });

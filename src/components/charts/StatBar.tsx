...
const StatBar: React.FC<StatBarProps> = ({ name, value, theme = 'dark' }) => {
  ...
  return (
    <div className="flex items-center gap-2 w-full">
      <span className={`w-1/3 text-sm font-semibold capitalize text-right ${theme === 'dark' ? 'text-slate-200' : 'text-slate-600'}`}>{name.replace('-', ' ')}</span>
      <div className="w-2/3 flex items-center gap-2">
        <span className={`font-bold w-8 ${theme === 'dark' ? 'text-white' : 'text-slate-800'}`}>{value}</span>
        <div className={`w-full h-2.5 rounded-full ${theme === 'dark' ? 'bg-black/20' : 'bg-slate-200'}`}>
          <div 

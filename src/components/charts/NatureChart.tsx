                 ...
                 <div className="overflow-y-auto pr-2 scrollbar-thin max-h-[70vh]">
                     <table className="w-full text-left border-collapse">
                         <thead className={`sticky top-0 z-10 ${theme === 'dark' ? 'bg-slate-900' : 'bg-white'}`}>
                             <tr className={`border-b ${theme === 'dark' ? 'border-white/20 text-slate-400' : 'border-slate-300 text-slate-600'}`}>
                         ...
                         <tbody className={`divide-y ${theme === 'dark' ? 'divide-white/10' : 'divide-slate-200'}`}>
                             {NATURES.map((nature, i) => (
                                 <tr key={i} className="hover:bg-white/5 transition-colors">
                                     <td className={`p-3 font-bold ${theme === 'dark' ? 'text-white' : 'text-slate-800'}`}>{nature.name}</td>
                                     <td className="p-3 text-green-500 font-semibold">{nature.up === 'None' ? '—' : nature.up}</td>
                                     <td className="p-3 text-red-500 font-semibold">{nature.down === 'None' ? '—' : nature.down}</td>
                                 </tr>
                             ))}
                         </tbody>
                     </table>

...
const CoverageGrid: React.FC<CoverageGridProps> = ({ team, theme }) => {
    ...
    return team.map((member) => {
      ...
      ALL_TYPES.forEach((type) => {
        if (mode === 'defensive') {
          ...
          const hasSelectedMoves =
            memberWithMoves.selectedMoves &&
            memberWithMoves.selectedMoves.some((m: string | null) => !!m);
  ...
  const getCellClass = (value: number) => {
    const base =
      'h-8 w-8 flex items-center justify-center text-xs border rounded transition-colors';
    const dark = theme === 'dark';

    if (mode === 'defensive') {
      if (value === 0)
        return `${base} ${dark ? 'bg-slate-800 border-slate-700' : 'bg-slate-200 border-slate-300'}`;
      if (value < 1)
        return `${base} ${dark ? 'bg-green-900/30 border-green-800/50' : 'bg-green-100 border-green-200'}`;
      if (value > 1)
        return `${base} ${dark ? 'bg-red-900/30 border-red-800/50' : 'bg-red-50 border-red-200'}`;
  ...
  return (
    <div
      className={`mt-4 overflow-hidden rounded-xl border ${theme === 'dark' ? 'bg-black/20 border-white/10' : 'bg-white border-slate-200'}`}
    >
      <div className="flex items-center justify-between p-3 border-b border-inherit">
        <h3
          className={`text-sm font-bold uppercase tracking-wider ${theme === 'dark' ? 'text-slate-300' : 'text-slate-600'}`}
        >
          Coverage Analysis
        </h3>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-center border-collapse">
          <thead>
            <tr>
              <th
                className={`p-2 sticky left-0 z-10 text-left min-w-[120px] ${theme === 'dark' ? 'bg-slate-900' : 'bg-white'}`}
              >
                Pokemon
              </th>
              {ALL_TYPES.map((t) => (
          ...
          <tbody className={`divide-y ${theme === 'dark' ? 'divide-white/5' : 'divide-slate-100'}`}>
            {gridData.map(({ member, row }) => (
              <tr key={member.id}>
                <td
                  className={`p-2 sticky left-0 z-10 text-left flex items-center gap-2 ${theme === 'dark' ? 'bg-slate-900' : 'bg-white'}`}
                >
                  <img
                  ...
                  />
                  <span
                    className={`text-xs font-semibold truncate max-w-[80px] ${theme === 'dark' ? 'text-slate-300' : 'text-slate-700'}`}
                  >
                    {member.name}
                  </span>
                </td>
                {ALL_TYPES.map((type) => (
                  <td key={type} className="p-1">
                    <div className="flex justify-center">
                      <div className={getCellClass(row[type])}>{getCellContent(row[type])}</div>
                    </div>
                  </td>
                ))}
              </tr>
            ...
            {/* Summary Row */}
            <tr
              className={`border-t-2 ${theme === 'dark' ? 'border-white/10 bg-white/5' : 'border-slate-200 bg-slate-50'}`}
            >
              <td
                className={`p-2 sticky left-0 z-10 text-left font-bold text-xs ${theme === 'dark' ? 'bg-slate-800 text-slate-400' : 'bg-slate-100 text-slate-500'}`}
              >
      ...
      <div
        className={`p-2 text-[10px] italic text-center ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`}
      >
        {mode === 'defensive'

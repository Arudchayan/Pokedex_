...
const AppHeader: React.FC<AppHeaderProps> = memo(({ onRandomPokemon }) => {
  ...
  return (
    <header
      className={`sticky top-0 z-sticky border-b backdrop-blur-xl ${
        isCyberpunk
          ? 'cyber-header'
          : theme === 'dark'
    ...
    >
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-2 px-3 py-2 sm:gap-4 sm:px-6 sm:py-4 md:gap-6 md:py-6">
        <div className="flex flex-col gap-2 sm:gap-3 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-2.5 sm:gap-4">
            ...
            {/* Primary utilities: tools menu + theme/shiny/audio (+ compare when active) */}
            <div className="flex gap-1.5 sm:gap-2 items-center ml-auto">
              ...
              <div className="hidden md:flex gap-2 items-center">
                <div className="relative" ref={menuRef}>
                  ...
                  {showMenu && (
                    <div
                      ...
                      role="menu"
                      aria-label="Tools"
                      className={`absolute top-full mt-2 right-0 w-56 rounded-xl shadow-2xl border p-2 z-50 flex flex-col gap-1 ${
                        theme === 'dark'
                    ...
                    >
                      ...
                      <div
                        className={`h-px w-full my-1 ${theme === 'dark' ? 'bg-white/10' : 'bg-slate-200'}`}
                      ></div>
                      <button
                        type="button"
                        role="menuitem"
                      ...
                      <button
                        ...
                        className={`p-2 text-left rounded-lg font-bold focus:outline-none focus:ring-2 focus:ring-primary-500 ${theme === 'dark' ? 'hover:bg-white/10 text-primary-400' : 'hover:bg-slate-100 text-primary-600'}`}
                      >
                        Start Walkthrough
                      </button>
                      <a
                ...
                <button
                  ...
                  className={`p-2 rounded-lg border transition-all hover:scale-105 focus-visible:ring-2 focus-visible:ring-primary-500 focus:outline-none ${
                    isShiny
                      ? 'bg-pink-500/20 border-pink-400/40 hover:bg-pink-500/30 text-pink-300'
                      : theme === 'dark'
                        ? 'bg-slate-800 border-slate-700 text-slate-400 hover:text-pink-300'
                        : 'bg-slate-200 border-slate-300 text-slate-500 hover:text-pink-500'
                  }`}
                ...
                {comparisonListLength > 0 && (
                  ...
                  >
                    <svg
                      className="w-5 h-5 text-primary-500 dark:text-primary-300"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >

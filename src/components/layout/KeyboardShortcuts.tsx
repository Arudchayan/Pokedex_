  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={`mt-6 text-xs text-center ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`}
        >
          Press <span className="font-mono font-bold bg-gray-500/20 px-1 rounded">Esc</span> to
          close
        </div>
      </div>
    </div>
...
const ShortcutRow: React.FC<{ shortcut: string; description: string; theme: string }> = ({
  shortcut,
  description,
  theme,
}) => (
  <div
    className={`flex items-center justify-between p-2 rounded-lg ${theme === 'dark' ? 'bg-white/5' : 'bg-slate-50'}`}
  >
    <span
      className={`font-mono text-sm font-bold px-2 py-1 rounded border ${theme === 'dark' ? 'bg-black/30 border-white/10 text-primary-300' : 'bg-white border-slate-200 text-primary-600'}`}
    >
      {shortcut}
    </span>

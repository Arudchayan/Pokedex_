...
const TeamMemberEditor: React.FC<TeamMemberEditorProps> = ({ member, onClose, onSave, theme }) => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        else if (pokemonData && pokemonData.abilities.length > 0) setAbility(pokemonData.abilities[0].name);
  ...
  const applyPreset = (type: 'physical' | 'special' | 'bulk' | 'fast_physical' | 'fast_special') => {
      const newEvs = { hp: 0, attack: 0, defense: 0, 'special-attack': 0, 'special-defense': 0, speed: 0 };

      switch(type) {
          case 'physical':
              newEvs.hp = 252; newEvs.attack = 252; newEvs.speed = 4;
              break;
          case 'special':
              newEvs.hp = 252; newEvs['special-attack'] = 252; newEvs.speed = 4;
              break;
          case 'fast_physical':
              newEvs.attack = 252; newEvs.speed = 252; newEvs.defense = 4;
              break;
          case 'fast_special':
              newEvs['special-attack'] = 252; newEvs.speed = 252; newEvs.defense = 4;
              break;
          case 'bulk':
              newEvs.hp = 252; newEvs.defense = 128; newEvs['special-defense'] = 128;
              break;
      }
      setEvs(newEvs);
  };
  ...
  const filteredItems = useMemo(() => {
      ...
      const searchLower = itemSearch.toLowerCase(); return items.filter(i => (i.nameLower || i.name.toLowerCase()).includes(searchLower)).slice(0, 10);
  }, [items, itemSearch]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="flex justify-between items-center p-6 border-b border-white/10 shrink-0">
            <div className="flex items-center gap-4">
                <div className="flex flex-col items-center gap-2">
                    <img
                        src={details ? (isShiny ? details.shinyImageUrl : details.imageUrl) : member.imageUrl}
                        className="w-16 h-16 object-contain"
                        alt={member.name}
                    />
                    <label className={`flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider cursor-pointer select-none ${theme === 'dark' ? 'text-slate-400 hover:text-white' : 'text-slate-500 hover:text-slate-700'}`}>
                        <input
                            type="checkbox"
                            checked={isShiny}
        ...
        {/* Tabs */}
        <div role="tablist" className={`flex border-b shrink-0 ${theme === 'dark' ? 'border-white/10' : 'border-slate-200'}`}>
            <button
                role="tab"
            ...
            {activeTab === 'main' && (
                <div role="tabpanel" id="panel-main" aria-labelledby="tab-main" className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        {/* Ability */}
                        <div>
                            <label className="block text-xs font-bold uppercase tracking-wider mb-2 opacity-70">Ability</label>
                            <select
                                value={ability}
                        ...
                        {/* Nature */}
                        <div>
                            <label className="block text-xs font-bold uppercase tracking-wider mb-2 opacity-70">Nature</label>
                            <select
                                value={nature}
                            ...
                            >
                                {Object.keys(NATURES).map(natureName => {
                                    ...
                                    const desc = n.up ? `(+${STAT_LABELS[n.up] || n.up}, -${STAT_LABELS[n.down!] || n.down})` : '(Neutral)';
                                    return <option key={natureName} value={natureName}>{natureName} {desc}</option>
                                })}
                            </select>
                        </div>
                        ...
                        <div className="md:col-span-2 relative">
                            <label className="block text-xs font-bold uppercase tracking-wider mb-2 opacity-70">Held Item</label>
                            <input
                                type="text"
                                value={itemSearch}
                                onChange={(e) => {
                            ...
                            {showItemDropdown && itemSearch && (
                                <div role="listbox" className={`absolute z-20 w-full mt-1 max-h-48 overflow-y-auto rounded-lg shadow-xl border ${theme === 'dark' ? 'bg-slate-800 border-white/10' : 'bg-white border-slate-200'}`}>
                                    {filteredItems.map(i => (
                                        <button
                                            type="button"
                                            role="option"
                                        ...
                                        >
                                            ...
                                            <span className="capitalize text-sm font-medium">{i.name.replace('-', ' ')}</span>
                                        </button>
                                    ))}
                                </div>
                            )}
                    ...
                    <hr className={`border-t ${theme === 'dark' ? 'border-white/10' : 'border-slate-200'}`} />
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {[0, 1, 2, 3].map(index => (
                                <div key={index}>
                                    <label className="block text-xs font-bold uppercase tracking-wider mb-1 opacity-50">Move {index + 1}</label>
                                    <select
                                        value={moves[index] || ''}
                ...
                <div role="tabpanel" id="panel-stats" aria-labelledby="tab-stats" className="space-y-6">
                    ...
                    <div className="flex flex-col md:flex-row gap-4 items-end justify-between">
                         <div className="w-full md:w-auto">
                            <label className="block text-xs font-bold uppercase tracking-wider mb-1 opacity-70">Level</label>
                            <input
                                type="number"
                                min="1"
                                max="100"
                         ...
                         <div className="w-full md:w-auto flex flex-col items-end">
                             <div className="text-xs font-bold uppercase tracking-wider mb-1 opacity-70">Quick Presets</div>
                             <div className="flex gap-2 flex-wrap justify-end">
                                 <button onClick={() => applyPreset('fast_physical')} className="px-2 py-1 text-xs border rounded hover:bg-primary-500/20">Fast Physical</button>
                                 <button onClick={() => applyPreset('fast_special')} className="px-2 py-1 text-xs border rounded hover:bg-primary-500/20">Fast Special</button>
                                 <button onClick={() => applyPreset('bulk')} className="px-2 py-1 text-xs border rounded hover:bg-primary-500/20">Bulky</button>
                             </div>
                         </div>
                    </div>
                    ...
                    <div className={`rounded-xl border overflow-hidden ${theme === 'dark' ? 'border-white/10' : 'border-slate-200'}`}>
                        <div className={`grid grid-cols-[1fr_50px_60px_1fr_60px] md:grid-cols-[1fr_60px_80px_2fr_80px] gap-2 p-3 font-bold text-xs uppercase tracking-wider border-b ${theme === 'dark' ? 'bg-white/5 border-white/10' : 'bg-slate-100 border-slate-200'}`}>
                            <div>Stat</div>
                        ...
                        {STAT_ORDER.map(stat => {
                            ...
                            return (
                                <div key={stat} className={`grid grid-cols-[1fr_50px_60px_1fr_60px] md:grid-cols-[1fr_60px_80px_2fr_80px] gap-2 items-center p-3 border-b last:border-0 ${theme === 'dark' ? 'border-white/5 hover:bg-white/5' : 'border-slate-100 hover:bg-slate-50'}`}>
                                    <div className="font-bold flex items-center gap-2">
                                        <span className={`w-3 h-3 rounded-full ${color.bar}`}></span>
                                        <span className="hidden md:inline capitalize">{stat.replace('special-', 'Sp. ')}</span>
        ...
        {/* Footer */}
        <div className={`p-6 border-t flex justify-end gap-3 shrink-0 ${theme === 'dark' ? 'border-white/10' : 'border-slate-200'}`}>
            <button
                onClick={onClose}

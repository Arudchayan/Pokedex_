  ...
  return (
    <Modal isOpen={true} onClose={onClose} title="Trainer Card" size="lg">
        <div className="mb-6 space-y-2">
            <label className={`block text-sm font-bold uppercase ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`}>Trainer Name</label>
            <input
                type="text"
                value={trainerName}
        ...
        >
            ...
            <div className="relative z-10 p-6 flex justify-between items-end border-b border-white/10 bg-black/20">
                <div>
                    <h1 className="text-4xl font-black tracking-tighter text-primary-400 uppercase drop-shadow-lg">{trainerName}</h1>
                    <div className="flex gap-4 mt-2 text-sm font-semibold opacity-70">
                        <span>ID No. {Math.floor(Math.random() * (UI_CONSTANTS.TRAINER_ID_MAX + 1)).toString().padStart(UI_CONSTANTS.TRAINER_ID_PADDING, '0')}</span>
                            ...
                            {pokemon ? (
                                <>
                                    <div className="absolute inset-0 opacity-20 transition-all duration-500 group-hover:opacity-30"
                                        style={{ background: `radial-gradient(circle at center, ${TYPE_COLORS_HEX[pokemon.types[0]]} 0%, transparent 70%)` }}
                                    ></div>
                                    <img
                                    ...
                                    <div className="absolute bottom-2 left-2 z-20">
                                         <p className="text-xs font-bold capitalize shadow-black drop-shadow-md">{pokemon.name}</p>
                                    </div>
                                    <div className="absolute top-2 right-2 z-20 flex gap-1">
                                        {pokemon.types.map(t => (
                                            <div key={t} className="w-3 h-3 rounded-full shadow-lg" style={{ backgroundColor: TYPE_COLORS_HEX[t] }}></div>
                                        ))}
                                    </div>
                                </>
                            ) : (

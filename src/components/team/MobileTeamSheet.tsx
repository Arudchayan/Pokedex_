...
interface MobileTeamSheetProps {
  ...
  /** Opens the full TeamBuilder (export, edit members, analytics) on mobile. */
        ...
        ReactDOM.createPortal(
          <div className="fixed inset-0 z-[1060] lg:hidden">
            ...
            >
              ...
              {/* Sheet header */}
              <div
                className={`flex items-center justify-between px-5 py-3 border-b ${isDark ? 'border-white/10' : 'border-slate-200'}`}
              >
                <div>
                ...
                {safeTeam.map((member) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p
                        className={`text-sm font-bold capitalize truncate ${isDark ? 'text-white' : 'text-slate-900'}`}
                      >
                        {member.name}
                      </p>
              ...
              {safeTeam.length > 0 && (
                <div
                  className={`mx-4 mb-4 p-3 rounded-xl border ${isDark ? 'border-white/10 bg-white/5' : 'border-slate-200 bg-slate-50'}`}
                >
                  <p
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {uniqueTypes.map((type) => (
                      ...
                      >
                        {type}{' '}
                        <span className={isDark ? 'text-primary-300' : 'text-primary-600'}>
                          x{typeCounts[type]}
                        </span>
                      </span>
                    ))}
              ...
              {/* Path to full TeamBuilder features (export, member editor, analytics, …) */}
              {onOpenFullBuilder && (
                <div className="px-4 pb-2">
                  ...
                  <p
                    className={`mt-2 text-center text-[11px] ${isDark ? 'text-slate-500' : 'text-slate-400'}`}
                  >
                    Edit movesets, export, share, and more
                  </p>
                </div>
              )}

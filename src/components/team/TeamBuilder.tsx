    ...
    >
      <header className="flex items-start justify-between gap-2">
        <div>
          ...
          }`}>{isCyberpunk ? 'Party' : 'Team'}</p>
          <h2 className={`text-xl font-bold ${
            isCyberpunk ? 'cyber-text-pink' : theme === 'dark' ? 'text-white' : 'text-slate-800'
          }`}>
            {team.length}/{teamCapacity}{' '}
            <span className={`text-sm font-medium ${
              isCyberpunk ? 'cyber-text opacity-70' : theme === 'dark' ? 'text-slate-400' : 'text-slate-500'
            }`}>slots</span>
          </h2>
          <p className={`text-xs ${
            isCyberpunk ? 'cyber-text opacity-60' : theme === 'dark' ? 'text-slate-400' : 'text-slate-500'
          }`}>
            {isTeamFull 
              ? (isCyberpunk ? 'Party is full! Move Pokemon to PC Box.' : 'Team is full. Remove a teammate to add another.') 
              : (isCyberpunk ? 'Select Pokemon to add to your party.' : 'Select cards to pin them here.')
            }
          </p>
        </div>
        <TeamBuilderActions
      ...
      <Suspense fallback={<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50"><Loader message="Loading..." /></div>}>
        {showExport && <TeamExportModal isOpen={showExport} onClose={() => setShowExport(false)} team={team} />}
      ...
      <div className={`mt-5 space-y-3 rounded-xl border p-3 ${
        theme === 'dark' ? 'border-white/10 bg-white/5' : 'border-slate-200 bg-slate-50'
      }`}>
        <div className="flex justify-between items-center">
          <p className={`text-xs font-semibold uppercase tracking-wide ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>Team Analytics</p>
          {team.length > 0 && (
            <button
        ...
        {team.length === 0 ? (
          <div className="flex flex-col items-center py-6">
            ...
            <svg className={`h-16 w-16 mb-3 ${theme === 'dark' ? 'text-slate-600' : 'text-slate-300'}`} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <p className={`text-xs mt-1 text-center ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`}>
          ...
          <>
            <div>
              <p className={`text-xs font-semibold mb-2 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>Type Coverage</p>
              <div className="flex flex-wrap gap-2">
                {uniqueTypes.map((type) => (
                  ...
                  >
                    {type}{' '}
                    <span className={theme === 'dark' ? 'text-primary-300' : 'text-primary-600'}>x{typeCounts[type]}</span>
                  </span>
                ))}
              </div>
            </div>
            ...
            {showAnalytics && (
              <Suspense fallback={<div className="p-4 flex justify-center"><Loader size="h-6 w-6" message="Loading analytics..." /></div>}>
                 {/* Coverage Grid */}
                 ...
                 {teamStats && (
                     <div className={`py-4 border-b ${theme === 'dark' ? 'border-white/10' : 'border-slate-200'}`}>
                         <p className={`text-xs font-semibold mb-2 text-center ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>Average Base Stats</p>
                 ...
                 {majorThreats.length > 0 && (
                     <div className="py-2">
                         ...
                         <p className={`text-[10px] mb-2 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>
                ...
                <div>
                  <p className={`text-xs font-semibold mb-2 mt-4 ${theme === 'dark' ? 'text-red-400' : 'text-red-600'}`}>
                ...
                <div>
                  <p className={`text-xs font-semibold mb-2 ${theme === 'dark' ? 'text-green-400' : 'text-green-600'}`}>
                ...
                <div>
                    <p className={`text-xs font-semibold mb-2 ${theme === 'dark' ? 'text-blue-400' : 'text-blue-600'}`}>
                             ...
                             {(Object.entries(offensiveCoverage) as Array<[string, number]>)
                                 ...
                                 .map(([type, count]) => (
                                     <div key={type} className="relative group/tooltip">
                                          <TypeBadge type={type} />
                                         {/* Tooltip */}
                                         <div className={`absolute bottom-full mb-1 left-1/2 -translate-x-1/2 px-2 py-1 text-[10px] rounded opacity-0 group-hover/tooltip:opacity-100 transition pointer-events-none whitespace-nowrap z-10 ${theme === 'dark' ? 'bg-black text-white' : 'bg-slate-800 text-white'}`}>
                                            Hit by {count} types
                                         </div>
                                    </div>
                                ))}
                ...
                <div className={`pt-2 border-t mt-2 ${theme === 'dark' ? 'border-white/10' : 'border-slate-200'}`}>
                  <p className={`text-xs ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>
                    💡 <span className="font-semibold">Tip:</span> Diversify types to cover more weaknesses!
                  </p>
                </div>
              </Suspense>
            )}

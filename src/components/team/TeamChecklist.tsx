...
const TeamChecklist: React.FC<TeamChecklistProps> = ({ team, theme }) => {
    ...
    Object.entries(CHECKLIST_CATEGORIES).forEach(([key, category]) => {
      ...
      team.forEach((member) => {
        if (member.selectedMoves) {
          const hasMove = member.selectedMoves.some(
            (moveName) => moveName && category.moves.includes(moveName.toLowerCase())
          );
          if (hasMove) {
            count++;
  ...
  return (
    ...
    >
      <div
        className={`px-4 py-3 border-b ${theme === 'dark' ? 'border-white/10' : 'border-slate-100'}`}
      >
        <h3
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-0.5">
                  <span
                    className={`text-xs font-bold uppercase tracking-wide ${theme === 'dark' ? 'text-slate-200' : 'text-slate-700'}`}
                  >
                    {category.label}
                  </span>
                  {count > 0 && (
                ...
                <p
                  className={`text-[10px] mb-1.5 truncate ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`}
                >
                ...
                {present ? (
                  ...
                  <span
                    className={`text-[10px] italic ${theme === 'dark' ? 'text-slate-600' : 'text-slate-400'}`}
                  >
                    Not detected
                  </span>
                )}

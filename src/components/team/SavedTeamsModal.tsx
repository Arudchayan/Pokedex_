...
const SavedTeamsModal: React.FC<SavedTeamsModalProps> = memo(
  ({ isOpen, onClose, currentTeam, onLoadTeam, theme }) => {
    ...
    return (
      <Modal isOpen={isOpen} onClose={onClose} title="Saved Teams" size="md">
        <div>
          ...
          <div
            className={`mb-6 p-4 rounded-xl border ${theme === 'dark' ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200'}`}
          >
            ...
            {currentTeam.length === 0 && (
              <p className="mt-2 text-xs opacity-60">Add Pokemon to your team to save it.</p>
            )}
          </div>
          ...
          <div className="space-y-3">
            ...
            {savedTeams.length === 0 ? (
              <div
                className={`text-center py-8 rounded-xl border border-dashed ${theme === 'dark' ? 'border-white/10 text-slate-500' : 'border-slate-300 text-slate-400'}`}
              >
                No saved teams yet.
              </div>
            ) : (
              savedTeams.map((saved) => (
                <div
                  key={saved.id}
                  className={`group flex flex-col gap-2 rounded-xl border p-3 transition-all ${
                    theme === 'dark'
                ...
                >
                  ...
                  <div className="flex gap-1 h-8">
                    ...
                    {Array.from({ length: 6 - saved.team.length }).map((_, i) => (
                      <div
                        key={i}
                        className={`h-8 w-8 rounded-full border border-dashed flex items-center justify-center ${theme === 'dark' ? 'border-white/10 bg-white/5' : 'border-slate-200 bg-slate-50'}`}
                      >
                  ...
                  <div className="flex gap-2 mt-2 pt-2 border-t border-dashed border-opacity-20 border-gray-400">
                    ...
                    <button
                      ...
                      aria-label={
                        deleteConfirmationId === saved.id ? 'Confirm deletion' : 'Delete team'
                      }
                    >

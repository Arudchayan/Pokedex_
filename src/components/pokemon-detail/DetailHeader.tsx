  ...
  <div className="relative p-8 rounded-t-2xl overflow-hidden">
    ...
    <div className="relative z-10">
      ...
      <div className="flex items-center gap-4 flex-wrap">
        <h2 className={`text-5xl font-extrabold capitalize ${theme === 'dark' ? 'text-white' : 'text-slate-800'}`}>
          {pokemon.name}
        </h2>
        <button
          onClick={onToggleShiny}
          className={`p-2 rounded-full transition-all duration-300 ${showShiny
        ...
        {(onAddToTeam || onRemoveFromTeam) && (
          <button
            ...
            className={`rounded-full px-4 py-2 text-sm font-semibold uppercase tracking-wide transition focus:outline-none focus:ring-2 focus:ring-primary-400/60 ${isInTeam
      ...
      <p className={`text-xl font-semibold mt-1 ${theme === 'dark' ? 'text-slate-200' : 'text-slate-600'}`}>
        {pokemon.genus}
      </p>

...
const StatCalculator: React.FC<StatCalculatorProps> = ({ onClose, initialPokemonId }) => {
  ...
  const applyPreset = (type: 'min' | 'max' | 'comp') => {
    if (type === 'min') {
      ...
      setIvs({ hp: 31, attack: 31, defense: 31, 'special-attack': 31, 'special-defense': 31, speed: 31 });
      setEvs({ hp: 252, attack: 252, defense: 252, 'special-attack': 252, 'special-defense': 252, speed: 252 });
    } else if (type === 'comp') {
        // Standard sweeping spread
        setIvs({ hp: 31, attack: 31, defense: 31, 'special-attack': 31, 'special-defense': 31, speed: 31 });
        setEvs({ hp: 4, attack: 252, defense: 0, 'special-attack': 0, 'special-defense': 0, speed: 252 });
        setNature('Jolly');
    }
  };
    ...
    <Modal isOpen={true} onClose={onClose} title="Stat Calculator" size="lg">
            ...
            {!pokemon ? (
                 <div className="relative z-20 max-w-md mx-auto mt-10">
                    <label htmlFor="pokemon-search" className="block text-sm font-bold mb-2 uppercase tracking-wider text-slate-500">Select Pokemon</label>
                    <input
                        id="pokemon-search"
                        type="text"
                    ...
                    {filteredList.length > 0 && (
                        <div className={`absolute top-full left-0 w-full z-10 rounded-b-xl shadow-xl max-h-60 overflow-y-auto mt-1 border ${theme === 'dark' ? 'bg-slate-800 border-white/10' : 'bg-white border-slate-200'}`}>
                            {filteredList.map(p => (
                                <div
                                    key={p.id}
                ...
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="flex items-center gap-6 mb-8">
                        ...
                        <div className="flex-1">
                            <div className="flex justify-between items-start">
                                ...
                                <button
                                    onClick={() => { setPokemon(null); setSelectedId(null); }}
                    ...
                    {/* Global Controls */}
                    <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 p-4 rounded-xl mb-6 ${theme === 'dark' ? 'bg-white/5 border border-white/5' : 'bg-slate-50 border border-slate-200'}`}>
                        <div>
                            <label htmlFor="level-slider" className="block text-xs font-bold uppercase tracking-wider mb-2 text-slate-500">Level</label>
                        ...
                        <div>
                            <label htmlFor="nature-select" className="block text-xs font-bold uppercase tracking-wider mb-2 text-slate-500">Nature</label>
                            <select
                                id="nature-select"
                                value={nature}
                            ...
                            >
                                {Object.keys(NATURES).map(n => (
                                    <option key={n} value={n}>
                                        {n}
                                        {NATURES[n].up ? ` (+${STAT_LABELS[NATURES[n].up || '']}, -${STAT_LABELS[NATURES[n].down || '']})` : ''}
                                    </option>
                                ))}
                            </select>
                        </div>
                    ...
                    <div className="flex flex-wrap gap-2 mb-6">
                        <button onClick={() => applyPreset('max')} className="px-3 py-1 text-xs font-bold rounded bg-primary-500/10 text-primary-500 hover:bg-primary-500 hover:text-white transition-colors">Max All (31/252)</button>
                        <button onClick={() => applyPreset('min')} className="px-3 py-1 text-xs font-bold rounded bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white transition-colors">Min All (0/0)</button>
                        <button onClick={() => applyPreset('comp')} className="px-3 py-1 text-xs font-bold rounded bg-purple-500/10 text-purple-500 hover:bg-purple-500 hover:text-white transition-colors">Phys. Sweeper</button>
                    </div>

                    {/* Stats Table */}
                    <div className={`overflow-hidden rounded-xl border ${theme === 'dark' ? 'border-white/10' : 'border-slate-200'}`}>
                        <div className={`grid grid-cols-[1fr_auto_auto_auto_1fr] gap-4 p-3 font-bold text-xs uppercase tracking-wider border-b ${theme === 'dark' ? 'bg-white/5 border-white/10 text-slate-400' : 'bg-slate-100 border-slate-200 text-slate-500'}`}>
                        ...
                        {STAT_ORDER.map(stat => {
                            ...
                            return (
                                <div key={stat} className={`grid grid-cols-[1fr_auto_auto_auto_1fr] gap-4 p-3 items-center border-b last:border-0 hover:bg-primary-500/5 transition-colors ${theme === 'dark' ? 'border-white/5' : 'border-slate-100'}`}>

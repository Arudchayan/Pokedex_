...
const DamageCalculator: React.FC<DamageCalculatorProps> = ({ onClose, initialAttackerId, initialDefenderId }) => {
  ...
  const filteredAttackerList = useMemo(() => {
      ...
      return masterPokemonList.filter(p => p.name.toLowerCase().includes(attackerSearch.toLowerCase())).slice(0, 5);
  }, [attackerSearch, masterPokemonList]);

  const filteredDefenderList = useMemo(() => {
      if (!defenderSearch) return [];
      return masterPokemonList.filter(p => p.name.toLowerCase().includes(defenderSearch.toLowerCase())).slice(0, 5);
  ...
  const StatControl = ({ pokemon, setPokemon, statName, label }: { pokemon: ExtendedBattlePokemon, setPokemon: any, statName: string, label: string }) => {
      return (
          <div className="flex items-center gap-2 text-xs">
              <span className={`w-8 font-bold uppercase ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>{label}</span>
              <div className="flex flex-col gap-1 flex-1">
                  <div className="flex items-center gap-2">
                       ...
                       <input
                           ...
                           onChange={e => setPokemon({...pokemon, ivs: { ...pokemon.ivs, [statName]: Math.min(31, Math.max(0, parseInt(e.target.value) || 0)) }})}
                       ...
                       <input
                           ...
                           onChange={e => setPokemon({...pokemon, evs: { ...pokemon.evs, [statName]: Math.min(252, Math.max(0, parseInt(e.target.value) || 0)) }})}
                           className={`w-12 p-1 rounded border text-center ${theme === 'dark' ? 'bg-black/30 border-white/10' : 'bg-white border-slate-300'}`}
                       />
                  </div>
              </div>
    ...
    <Modal isOpen={true} onClose={onClose} title="Damage Calculator" size="xl">
        <div className="flex-1 overflow-y-auto p-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {/* Attacker Column */}
                <div className={`p-4 rounded-xl border ${theme === 'dark' ? 'bg-white/5 border-white/5' : 'bg-slate-50 border-slate-200'}`}>
                    ...
                    {!attacker ? (
                        <div className="relative z-20">
                            ...
                            {filteredAttackerList.length > 0 && (
                                <div className={`absolute top-full left-0 w-full z-10 rounded-b shadow-xl max-h-60 overflow-y-auto ${theme === 'dark' ? 'bg-slate-800' : 'bg-white'}`}>
                                    {filteredAttackerList.map(p => (
                                        <div
                                            key={p.id}
                                            className="p-2 hover:bg-primary-500/20 cursor-pointer flex items-center gap-2"
                                            onClick={() => { setAttackerId(p.id); setAttackerSearch(''); }}
                                        >
                        ...
                        <div className="space-y-4">
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <button onClick={() => setAttacker(null)} className="text-xs text-red-400 hover:underline">Change</button>
                                    </div>
                                    <div className="flex gap-1 mt-1">
                                        {attacker.types.map(t => <TypeBadge key={t} type={t} />)}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setAttacker({...attacker, level: parseInt(e.target.value) || 1})}
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => setAttacker({...attacker, selectedAbility: e.target.value})}
                                        className={`w-full p-2 rounded border mt-1 ${theme === 'dark' ? 'bg-black/20 border-white/10' : 'bg-white border-slate-300'}`}
                                    >
                            ...
                            {showAttackerStats && (
                                <div className={`p-3 rounded space-y-2 animate-in fade-in slide-in-from-top-2 ${theme === 'dark' ? 'bg-black/20' : 'bg-slate-100'}`}>
                                    <StatControl pokemon={attacker} setPokemon={setAttacker} statName="hp" label="HP" />
                                    <StatControl pokemon={attacker} setPokemon={setAttacker} statName="attack" label="Atk" />
                                    <StatControl pokemon={attacker} setPokemon={setAttacker} statName="defense" label="Def" />
                                    <StatControl pokemon={attacker} setPokemon={setAttacker} statName="special-attack" label="SpA" />
                                    <StatControl pokemon={attacker} setPokemon={setAttacker} statName="special-defense" label="SpD" />
                                    <StatControl pokemon={attacker} setPokemon={setAttacker} statName="speed" label="Spe" />
                                </div>
                            )}
                            ...
                            {attacker.selectedMove && (
                                 <div className={`p-3 rounded border ${theme === 'dark' ? 'bg-black/20 border-white/5' : 'bg-slate-100 border-slate-200'}`}>
                                     <div className="flex justify-between text-sm">
                                         <span>Type: <span className="capitalize font-bold" style={{color: TYPE_COLORS[attacker.selectedMove.type]}}>{attacker.selectedMove.type}</span></span>
                                         <span>Power: {attacker.selectedMove.power}</span>
                                         <span>Class: <span className="capitalize">{attacker.selectedMove.damageClass}</span></span>
                                     </div>
                                 </div>
                            )}
                        </div>
                                ...
                                <div className={`absolute top-full left-0 w-full z-10 rounded-b shadow-xl max-h-60 overflow-y-auto ${theme === 'dark' ? 'bg-slate-800' : 'bg-white'}`}>
                                    {filteredDefenderList.map(p => (
                                        <div
                                            ...
                                            onClick={() => { setDefenderId(p.id); setDefenderSearch(''); }}
                                        >
                                            <img src={p.imageUrl} className="w-8 h-8" />
                                            <span>{p.name}</span>
                                        </div>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <button onClick={() => setDefender(null)} className="text-xs text-red-400 hover:underline">Change</button>
                                    </div>
                                    <div className="flex gap-1 mt-1">
                                        {defender.types.map(t => <TypeBadge key={t} type={t} />)}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setDefender({...defender, level: parseInt(e.target.value) || 1})}
                                        className={`w-full p-2 rounded border mt-1 ${theme === 'dark' ? 'bg-black/20 border-white/10' : 'bg-white border-slate-300'}`}
                                    />
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => setDefender({...defender, selectedAbility: e.target.value})}
                                        className={`w-full p-2 rounded border mt-1 ${theme === 'dark' ? 'bg-black/20 border-white/10' : 'bg-white border-slate-300'}`}
                                    >
                                        {defender.abilities.map(a => <option key={a} value={a}>{a}</option>)}
                                    </select>
                                </div>
                            </div>
                             ...
                             {showDefenderStats && (
                                <div className={`p-3 rounded space-y-2 animate-in fade-in slide-in-from-top-2 ${theme === 'dark' ? 'bg-black/20' : 'bg-slate-100'}`}>
                                    <StatControl pokemon={defender} setPokemon={setDefender} statName="hp" label="HP" />
                                    <StatControl pokemon={defender} setPokemon={setDefender} statName="attack" label="Atk" />
                                    <StatControl pokemon={defender} setPokemon={setDefender} statName="defense" label="Def" />
                                    <StatControl pokemon={defender} setPokemon={setDefender} statName="special-attack" label="SpA" />
                                    <StatControl pokemon={defender} setPokemon={setDefender} statName="special-defense" label="SpD" />
                                    <StatControl pokemon={defender} setPokemon={setDefender} statName="speed" label="Spe" />
                                </div>
                            )}
                        </div>
                    )}
        ...
        <div className={`p-6 border-t flex-shrink-0 ${theme === 'dark' ? 'border-white/10 bg-slate-900' : 'border-slate-200 bg-white'} rounded-b-2xl`}>
            {result ? (
                <div className="text-center space-y-4">
                    ...
                    <div className="flex justify-center gap-2 mt-4 flex-wrap">
                        {result.modifiers.stab && (
                             <span className={`px-3 py-1 rounded-full text-xs font-bold border ${theme === 'dark' ? 'bg-blue-500/20 text-blue-300 border-blue-500/30' : 'bg-blue-100 text-blue-700 border-blue-200'}`}>
                                 STAB
                             </span>
                        )}
                        {result.modifiers.typeEffectiveness > 1 && (
                             <span className={`px-3 py-1 rounded-full text-xs font-bold border ${theme === 'dark' ? 'bg-red-500/20 text-red-300 border-red-500/30' : 'bg-red-100 text-red-700 border-red-200'}`}>
                        ...
                        {result.modifiers.typeEffectiveness < 1 && result.modifiers.typeEffectiveness > 0 && (
                             <span className={`px-3 py-1 rounded-full text-xs font-bold border ${theme === 'dark' ? 'bg-orange-500/20 text-orange-300 border-orange-500/30' : 'bg-orange-100 text-orange-700 border-orange-200'}`}>
                        ...
                        {result.modifiers.item > 1 && (
                            <span className={`px-3 py-1 rounded-full text-xs font-bold border ${theme === 'dark' ? 'bg-purple-500/20 text-purple-300 border-purple-500/30' : 'bg-purple-100 text-purple-700 border-purple-200'}`}>

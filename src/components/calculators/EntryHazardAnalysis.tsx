  ...
  const analysis = useMemo(() => {
    return team.map((member) => {
      ...
      const hasMagicGuard =
        ...
        (!member.selectedAbility && possibleAbilities.includes('magic guard'));
      const hasLevitate =
        abilityName === 'levitate' ||
        (!member.selectedAbility && possibleAbilities.includes('levitate'));
      const hasAirBalloon = item === 'air balloon';
      const hasClearBody =
        abilityName === 'clear body' ||
        (!member.selectedAbility && possibleAbilities.includes('clear body'));
      const hasWhiteSmoke =
        abilityName === 'white smoke' ||
        (!member.selectedAbility && possibleAbilities.includes('white smoke')); // similar to Clear Body
      // Full Metal Body?
      ...
      return {
        ...
        stealthRock: {
          ...
          status: stealthRockStatus,
          weakness: rockWeakness,
        },
    ...
    >
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr
              className={`text-xs uppercase tracking-wider ${theme === 'dark' ? 'text-slate-500 bg-white/5' : 'text-slate-400 bg-slate-50'}`}
            >
          ...
          <tbody
            className={`divide-y text-xs font-medium ${theme === 'dark' ? 'divide-white/5' : 'divide-slate-100'}`}
          >
            {analysis.map(({ member, stealthRock, spikes, toxicSpikes, stickyWeb }) => (
              ...
              >
                ...
                <td className="p-3 text-center">
                  <span
                    className={`px-2 py-1 rounded ${
                      stealthRock.status === 'Immune'
                        ? theme === 'dark'
                          ? 'bg-green-500/20 text-green-400'
                          : 'bg-green-100 text-green-700'
                        : stealthRock.damage >= 25
                          ? theme === 'dark'
                ...
                <td className="p-3 text-center">
                  <span
                    className={`px-2 py-1 rounded ${
                      toxicSpikes.status === 'Absorbs'
                        ? theme === 'dark'
                          ? 'bg-purple-500/20 text-purple-400 font-bold'
                          : 'bg-purple-100 text-purple-700 font-bold'
                        : toxicSpikes.status === 'Immune'
                          ? theme === 'dark'
                            ? 'bg-green-500/20 text-green-400'
                            : 'bg-green-100 text-green-700'
                          : theme === 'dark'
      ...
      >
        Calculations account for Types, Items (e.g. Heavy-Duty Boots, Air Balloon), and Abilities
        (e.g. Levitate, Magic Guard).
      </div>
    </div>
  );
};

...
import { GENERATIONS, SHINY_METHODS, GenerationKey, calculateShinyOdds } from '../../utils/shinyOddsUtils';
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title="Shiny Odds Calculator" size="md">
        <div className="space-y-6">
            ...
            <div>
                <label className="block text-sm font-bold uppercase tracking-wider mb-2 opacity-70">Game Generation</label>
                <select
                    value={generation}
            ...
            {/* Method Selection */}
            <div>
                <label className="block text-sm font-bold uppercase tracking-wider mb-2 opacity-70">Hunting Method</label>
            ...
            <div className="space-y-3">
                 <label className="block text-sm font-bold uppercase tracking-wider mb-2 opacity-70">Modifiers</label>

                 {generation !== 'gen2' && (
                     <label className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${hasCharm ? 'bg-yellow-500/10 border-yellow-500/50' : 'border-transparent bg-black/5 hover:bg-black/10'}`}>
                        <input
                            type="checkbox"
                            checked={hasCharm}
                        ...
                        <div className="flex-1">
                            <span className={`font-bold block ${hasCharm ? 'text-yellow-500' : ''}`}>Shiny Charm</span>
                            <span className="text-xs opacity-60">Key Item that increases shiny odds drastically.</span>
                        </div>
                 ...
                 {generation === 'gen9' && (
                     <label className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${hasSandwich ? 'bg-orange-500/10 border-orange-500/50' : 'border-transparent bg-black/5 hover:bg-black/10'}`}>
                        <input
                            type="checkbox"
                            checked={hasSandwich}
                        ...
                        />
                         <div className="flex-1">
                            <span className={`font-bold block ${hasSandwich ? 'text-orange-500' : ''}`}>Sandwich Power Lv. 3</span>
            ...
            {/* Result Display */}
            <div className={`mt-6 p-6 rounded-xl border text-center relative overflow-hidden ${theme === 'dark' ? 'bg-black/40 border-white/10' : 'bg-slate-100 border-slate-200'}`}>
                 ...
                 <p className="text-sm uppercase tracking-widest opacity-60 font-bold mb-1">Estimated Odds</p>

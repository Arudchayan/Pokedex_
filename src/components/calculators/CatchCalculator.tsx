...
import { BALL_TYPES, STATUS_CONDITIONS, calculateCatchProbability, getBallMultiplier } from '../../utils/catchUtils';
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            <div className="flex justify-between mb-2">
                                <label className="text-sm font-bold uppercase tracking-wider text-slate-500">Target HP</label>
                                <span className={`font-mono font-bold ${hpPercent < 30 ? 'text-red-500' : 'text-green-500'}`}>{hpPercent}%</span>
                            </div>
                            <input
                                type="range"
                                min="1"
                        ...
                        {/* Ball Selector */}
                        <div>
                            <label className="text-sm font-bold uppercase tracking-wider text-slate-500 block mb-2">Poke Ball</label>
                            <select
                        ...
                        {/* Status Selector */}
                        <div>
                            <label className="text-sm font-bold uppercase tracking-wider text-slate-500 block mb-2">Status Condition</label>
                    ...
                    <div className="flex flex-col items-center justify-center p-6 rounded-2xl bg-gradient-to-br from-black/5 to-black/10 border border-white/5 relative overflow-hidden">
                         ...
                         <div className="relative z-10 text-center space-y-2">
                             <p className="text-slate-400 font-bold uppercase tracking-widest text-sm">Capture Probability</p>
                             <div className={`text-7xl font-black tracking-tighter transition-colors duration-500 ${probabilityColor}`}>
                         ...
                         <div className="mt-8 w-full space-y-3">
                             {selectedBall === 'NET_BALL' && !pokemon.types.includes('water') && !pokemon.types.includes('bug') && (
                                 <div className="p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-lg flex items-start gap-3">
                                     <span className="text-xl">⚠️</span>
                                     <p className="text-xs text-yellow-200">Net Ball is only effective on Water or Bug types (x3.5). Currently acting as a Poke Ball (x1).</p>
                                 </div>
                             )}
                              {selectedBall === 'DUSK_BALL' && (
                                 <label className="flex items-center gap-3 p-3 bg-black/20 rounded-lg cursor-pointer hover:bg-black/30 transition-colors">
                                     <input type="checkbox" checked={isNight} onChange={e => setIsNight(e.target.checked)} className="rounded text-primary-500 focus:ring-primary-500 bg-transparent border-slate-500" />
                              ...
                              {selectedBall === 'QUICK_BALL' && (
                                 <label className="flex items-center gap-3 p-3 bg-black/20 rounded-lg cursor-pointer hover:bg-black/30 transition-colors">
                                     <input type="checkbox" checked={turn === 1} onChange={e => setTurn(e.target.checked ? 1 : 2)} className="rounded text-primary-500 focus:ring-primary-500 bg-transparent border-slate-500" />
                                     <div className="text-sm">

  ...
  return (
    ...
    >
      ...
      <div className="text-center px-1 w-full">
        <p
          className={`text-xs font-bold truncate ${theme === 'dark' ? 'text-slate-200' : 'text-slate-700'}`}
        >
          {evo.name}
        </p>
      </div>
...
const ResponsiveGraphNode: React.FC<{
  ...
  return (
    <li
      className={`flex ${isRoot ? 'items-center' : ''} ${isRoot ? 'py-0' : 'py-2 relative items-center'}`}
    >
      {/* Vertical Line Connector Logic (Spine for siblings) */}
      {!isRoot && totalSiblings > 1 && (
        <div
          className={`absolute -left-[1px] w-0.5 ${theme === 'dark' ? 'bg-slate-600' : 'bg-slate-300'}`}
          style={{
            height: '100%',
            top: index === 0 ? '50%' : '0',
      ...
      <div className="flex flex-row items-center">
        ...
        {!isRoot && (
          <div className="w-12 sm:w-16 relative flex items-center justify-center">
            <div
              className={`h-0.5 w-full ${theme === 'dark' ? 'bg-slate-600' : 'bg-slate-300'}`}
            ></div>
            <TriggerLabel
              evo={node.data}
              theme={theme}
        ...
        {node.children.length > 0 && (
          <div className="flex flex-row items-center">
            ...
            <div
              className={`w-8 sm:w-12 h-0.5 ${theme === 'dark' ? 'bg-slate-600' : 'bg-slate-300'}`}
            ></div>
...
const EvolutionGraph: React.FC<EvolutionGraphProps> = ({
  ...
  return (
    <div
      className={`w-full overflow-x-auto p-4 rounded-xl ${theme === 'dark' ? 'bg-black/20' : 'bg-slate-50/50'}`}
    >
      ...
      <div
        className={`text-center mt-4 text-xs italic ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`}
      >

...
const MoveRecommender: React.FC<MoveRecommenderProps> = ({ moves, stats, types }) => {
  ...
  return (
    <div
      className={`mt-6 p-4 rounded-xl border ${theme === 'dark' ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200'}`}
    >
      <h4
        className={`text-lg font-bold mb-4 flex items-center gap-2 ${theme === 'dark' ? 'text-yellow-400' : 'text-yellow-600'}`}
      >
        <svg
        ...
        {recommendedMoves.map((move) => (
          ...
          >
            <div className="flex justify-between items-start mb-2">
              <div>
                <span className="font-bold capitalize block text-lg">
                  {move.name.replace('-', ' ')}
                </span>
            ...
            <div className="flex flex-wrap gap-1 mt-1">
              {move.reason.map((r, i) => (
                <span
                  key={i}
                  className={`text-[10px] px-1.5 py-0.5 rounded border ${theme === 'dark' ? 'border-yellow-500/30 text-yellow-200 bg-yellow-500/10' : 'border-yellow-600/30 text-yellow-700 bg-yellow-100'}`}
                >
                  {r}
                </span>
              ))}
      ...
      <p
        className={`text-xs mt-3 text-center opacity-60 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}
      >

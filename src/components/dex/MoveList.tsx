...
const DamageClassIcon: React.FC<{ damageClass: string }> = ({ damageClass }) => {
    ...
    return <span className={`${color} text-lg`} title={title} role="img" aria-label={title}>{icon}</span>
}
...
const MoveList: React.FC<MoveListProps> = ({ moves, onOpenMoveDex }) => {
  ...
  const groupedMoves = useMemo(() => {
    return moves.reduce((acc, move) => {
      const key = move.learnMethod === "level up" ? "Level Up" : 
                  move.learnMethod === "machine" ? "Machine" : 
                  move.learnMethod === "egg" ? "Egg" : "Other";
      if (!acc[key]) {
        acc[key] = [];
      }
      acc[key].push(move);
  ...
  const learnMethods = useMemo(() => ['Level Up', 'Machine', 'Egg', 'Other'].filter(method => groupedMoves[method]), [groupedMoves]);
  const [activeTab, setActiveTab] = useState(learnMethods[0] || '');

  const movesForActiveTab = groupedMoves[activeTab] || [];

  const filteredMoves = useMemo(() => {
      return movesForActiveTab.filter(move =>
    ...
    <div className="bg-black/10 rounded-lg overflow-hidden">
        ...
        <div className="overflow-x-auto max-h-[400px] scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
            <table className="min-w-full divide-y divide-white/10">
                ...
                <tr>
                    <SortableHeader title="Lvl" sortKey="level" sortConfig={sortConfig} onSort={requestSort} />
                    <SortableHeader title="Move" sortKey="name" sortConfig={sortConfig} onSort={requestSort} />
                    <SortableHeader title="Type" sortKey="type" sortConfig={sortConfig} onSort={requestSort} />
                    <th className="p-2 sm:p-3 text-left text-xs sm:text-sm font-bold text-slate-300 uppercase tracking-wider">Cat.</th>
                    <SortableHeader title="Pow" sortKey="power" sortConfig={sortConfig} onSort={requestSort} />
                    <SortableHeader title="Acc" sortKey="accuracy" sortConfig={sortConfig} onSort={requestSort} />
                    <SortableHeader title="PP" sortKey="pp" sortConfig={sortConfig} onSort={requestSort} />
                    {activeTab === 'Other' && <SortableHeader title="Method" sortKey="learnMethod" sortConfig={sortConfig} onSort={requestSort} />}
                </tr>
                </thead>
                ...
                {sortedMoves.map((move, index) => (
                    ...
                    <td className="p-2 sm:p-3 whitespace-nowrap"><TypeBadge type={move.type} /></td>
                    <td className="p-2 sm:p-3 whitespace-nowrap text-center"><DamageClassIcon damageClass={move.damageClass}/></td>
                    ...
                    {activeTab === 'Other' && <td className="p-2 sm:p-3 whitespace-nowrap text-xs sm:text-sm capitalize text-slate-400">{move.learnMethod}</td>}
                    </tr>
                ))}
                </tbody>
            </table>

  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-6 w-full">
        ...
        <h2 className="text-2xl font-black uppercase tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-blue-500">
          Who's That Pokemon?
        </h2>
      ...
      <div className="flex-1 flex flex-col items-center justify-center w-full max-w-md">
        ...
        {hintRevealed && !revealed && (
          <div className="mb-6 flex gap-2 animate-fade-in">
            {target.types.map((t) => (
              <span
                key={t}
                className="px-3 py-1 rounded-full text-xs font-bold uppercase bg-slate-500 text-white"
              >
                {t}
              </span>
            ))}
          ...
          {options.map((opt) => {
            ...
            if (revealed) {
              if (opt.id === target.id) btnClass = 'bg-green-500 text-white border-green-400';
                ...
                btnClass = theme === 'dark' ? 'bg-white/5 opacity-50' : 'bg-slate-100 opacity-50';
            } else {
              btnClass =
                theme === 'dark'
        ...
        {revealed && (
          <div
            className={`mt-8 text-xl font-bold ${guessState === 'correct' ? 'text-green-500' : 'text-red-500'}`}
          >

...
const PokedleGame: React.FC<Props> = ({ onClose, date, seed }) => {
    ...
    return (
        <div className={`flex flex-col h-full w-full ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>
            {/* Header */}
            <div className="flex justify-between items-center mb-6">
                <button onClick={onClose} className="text-sm font-bold opacity-70 hover:opacity-100">← Back</button>
                <h2 className="text-2xl font-black uppercase tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-cyan-500">Pokedle</h2>
            ...
            {gameState === 'playing' && (
                <div className="relative mb-6 z-20">
                    ...
                    />
                    {searchList.length > 0 && (
                        <div className={`absolute top-full left-0 w-full mt-2 rounded-xl shadow-2xl max-h-60 overflow-y-auto ${theme === 'dark' ? 'bg-slate-800' : 'bg-white'}`}>
                            {searchList.map(p => (
                                <button
                                    key={p.id}
            ...
            {gameState !== 'playing' && (
                <div className={`p-6 rounded-xl text-center mb-6 ${gameState === 'won' ? 'bg-green-500/20 border-green-500' : 'bg-red-500/20 border-red-500'} border-2`}>
                    <h3 className="text-2xl font-black mb-2">{gameState === 'won' ? 'You Caught It!' : 'It Got Away!'}</h3>
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pr-2">
                {guesses.map((g, i) => (
                    <div key={i} className="grid grid-cols-9 gap-1 h-14 animate-fade-in-up">
                        ...
                        {/* Type 1 */}
                        <div className={`col-span-1 flex items-center justify-center rounded-lg border text-[10px] font-bold capitalize leading-none ${g.attributes.type1 === 'correct' ? 'bg-green-500 border-green-400' : g.attributes.type1 === 'partial' ? 'bg-yellow-500 border-yellow-400' : 'bg-red-500 border-red-400'}`}>
                            {g.pokemon.types[0]}
                        </div>

                        {/* Type 2 */}
                        <div className={`col-span-1 flex items-center justify-center rounded-lg border text-[10px] font-bold capitalize leading-none ${g.attributes.type2 === 'correct' ? 'bg-green-500 border-green-400' : g.attributes.type2 === 'partial' ? 'bg-yellow-500 border-yellow-400' : 'bg-red-500 border-red-400'}`}>
                            {g.pokemon.types[1] || 'none'}
                        </div>

                        {/* Habitat */}
                        <div className={`col-span-1 flex items-center justify-center rounded-lg border text-[10px] font-bold capitalize leading-none ${g.attributes.habitatMatch ? 'bg-green-500 border-green-400' : 'bg-red-500 border-red-400'}`}>
                            {g.pokemon.habitat || '-'}
                        </div>

                        {/* Stage */}
                        <div className={`col-span-1 flex items-center justify-center rounded-lg border font-bold ${g.attributes.stageDir === 'correct' ? 'bg-green-500 border-green-400' : 'bg-red-500 border-red-400'}`}>
                            {getStage(g.pokemon)}
                        ...
                        {/* Gen */}
                        <div className={`col-span-1 flex items-center justify-center rounded-lg border font-bold ${g.attributes.genDir === 'correct' ? 'bg-green-500 border-green-400' : 'bg-red-500 border-red-400'}`}>
                            {getGen(g.pokemon.id)}
                        ...
                        {/* Height */}
                        <div className={`col-span-1 flex items-center justify-center rounded-lg border font-bold ${g.attributes.heightDir === 'correct' ? 'bg-green-500 border-green-400' : 'bg-red-500 border-red-400'}`}>
                            {g.pokemon.height}
                        ...
                        {/* Weight */}
                        <div className={`col-span-1 flex items-center justify-center rounded-lg border font-bold ${g.attributes.weightDir === 'correct' ? 'bg-green-500 border-green-400' : 'bg-red-500 border-red-400'}`}>
                            {g.pokemon.weight}
                        ...
                        {/* Color */}
                        <div className={`col-span-1 flex items-center justify-center rounded-lg border font-bold capitalize text-[10px] ${g.attributes.colorMatch ? 'bg-green-500 border-green-400' : 'bg-red-500 border-red-400'}`}>
                            {g.pokemon.color}
                        </div>
                    </div>
                ))}

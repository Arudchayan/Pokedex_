    ...
    return (
        <div className={`flex flex-col h-full w-full items-center ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>
            <div className="flex justify-between items-center mb-6 w-full">
                ...
                <h2 className="text-2xl font-black uppercase tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-red-500">StatDle</h2>
                        ...
                        <div className="flex justify-center mb-4">
                            {hintRevealed ? (
                                <div className="px-4 py-2 bg-orange-500/20 text-orange-600 dark:text-orange-400 border border-orange-500/50 rounded-xl font-bold animate-fade-in text-sm flex gap-2">
                                    ...
                                    <span className="uppercase">{target.stats.reduce((prev, current) => (prev.value > current.value) ? prev : current).name}</span>
                                </div>
                            ) : (
                                <button
                                    onClick={() => handleHint()}

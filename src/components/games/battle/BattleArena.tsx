...
const HpBar: React.FC<{ current: number; max: number; label: string }> = ({ current, max, label }) => {
    ...
    return (
        <div className="w-full mb-2">
            ...
            <div className="text-right text-[10px] opacity-70">{current}/{max}</div>
        </div>
    );
};
...
const BattleArena: React.FC<Props> = ({ playerActive, enemyActive }) => {
    ...
    return (
        <div className={`relative w-full aspect-video rounded-xl overflow-hidden border-2 ${theme === 'dark' ? 'bg-slate-800 border-slate-700' : 'bg-slate-100 border-slate-200'}`}>
            ...
            <div className="absolute inset-0 p-4 pointer-events-none">
                ...
                <div className="absolute top-4 left-4 w-48 bg-white/80 dark:bg-black/50 p-2 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
                    ...
                    {enemyActive.status !== 'none' && (
                        <span className="px-1 bg-purple-500 text-white text-[10px] rounded uppercase">{enemyActive.status}</span>
                    )}
                </div>
                ...
                <div className="absolute bottom-8 right-4 w-48 bg-white/80 dark:bg-black/50 p-2 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700">
                    ...
                    {playerActive.status !== 'none' && (
                        <span className="px-1 bg-purple-500 text-white text-[10px] rounded uppercase">{playerActive.status}</span>
                    )}
                </div>
            </div>
        </div>

...
const BattleGame: React.FC<Props> = ({ onClose }) => {
    ...
    const handleStart = async () => {
        ...
        try {
            ...
            try {
                ...
                const timeout = new Promise((_, rej) => setTimeout(() => rej(new Error("Timeout")), TIMEOUT_DELAYS.BATTLE_TEAM_GENERATION));
    ...
    const handleMove = async (moveIndex: number) => {
        ...
        const playerAction = { type: 'move' as const, actorId: battle.state.playerSide.active!.id, moveIndex };
    ...
    const handleSwitch = async (switchTargetIndex: number) => {
        ...
        await new Promise(r => setTimeout(r, 10)); // Yield
        const aiAction = ai.getBestAction(1);
        const playerAction = { type: 'switch' as const, actorId: battle.state.playerSide.active!.id, switchTargetIndex };
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center p-12 text-center">
                ...
                <div>{error}</div>
                <button onClick={onClose} className="mt-4 px-4 py-2 bg-slate-500 text-white rounded">Close</button>
            </div>
        );
    }
        ...
        return (
            <div className="flex flex-col items-center justify-center h-full p-8 text-center animate-fade-in-up">
                <h2 className="text-4xl font-black mb-4">
                    {pAlive ? <span className="text-green-500">You Won!</span> : <span className="text-red-500">You Lost!</span>}
                </h2>
                <button onClick={onClose} className="px-6 py-3 bg-slate-500 text-white rounded-xl font-bold">Return to Hub</button>
            </div>
        );
    }

    return (
        <div className={`flex flex-col h-full max-w-4xl mx-auto p-2 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>
            <div className="flex justify-between items-center mb-2">
                <button onClick={onClose} className="text-xs font-bold opacity-50 hover:opacity-100">← Exit Battle</button>

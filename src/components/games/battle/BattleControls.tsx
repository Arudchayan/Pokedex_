...
const BattleControls: React.FC<Props> = ({ activePokemon, playerTeam, onMove, onSwitch, onRun, disabled }) => {
    ...
    const switchOptions = useMemo(() => {
        return displayTeam.map((member) => {
            const battleIndex = playerTeam.findIndex((mon) =>
                mon.speciesId === member.id || mon.name.toLowerCase() === member.name.toLowerCase()
            );
    ...
    return (
        <div className="grid grid-cols-2 gap-4 mt-4">
            {activePokemon.moves.map((move, idx) => (
                ...
                >
                    <div className={`absolute top-0 right-0 px-2 py-1 text-xs font-bold uppercase rounded-bl-lg text-white ${TYPE_COLORS[move.type] || 'bg-slate-500'}`}>
                        {move.type}
                    </div>

...
const MoveGame: React.FC<Props> = ({ onClose, date, seed }) => {
    ...
    if (error) {
        return (
            <div className={`text-center p-8 flex flex-col items-center justify-center h-full ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>
                <p className="text-red-500 font-bold mb-4">{error}</p>
                <button onClick={onClose} className="px-4 py-2 bg-slate-500 text-white rounded-lg">Go Back</button>
            </div>
        );
    }

    if (isLoading || !target) return <div className={`text-center p-8 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Loading MoveDle...</div>;
    ...
    return (
        <div className={`flex flex-col h-full w-full ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>
            <div className="flex justify-between items-center mb-6">
                ...
                <h2 className="text-2xl font-black uppercase tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-yellow-500">MoveDle</h2>
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pr-2">
                {guesses.map((g, i) => (
                    <div key={i} className="grid grid-cols-6 gap-2 h-14 animate-fade-in-up">
                        ...
                        <div className={`col-span-1 flex items-center justify-center rounded-lg border text-[10px] font-bold capitalize ${g.attributes.type === 'correct' ? 'bg-green-500 border-green-400' : 'bg-red-500 border-red-400'}`}>
                            {g.move.type}
                        </div>

                        <div className={`col-span-1 flex items-center justify-center rounded-lg border text-[10px] font-bold capitalize ${g.attributes.category === 'correct' ? 'bg-green-500 border-green-400' : 'bg-red-500 border-red-400'}`}>
                            {g.move.category}
                        </div>

                        <div className={`col-span-1 flex items-center justify-center rounded-lg border font-bold ${g.attributes.power === 'correct' ? 'bg-green-500 border-green-400' : 'bg-red-500 border-red-400'}`}>
                            {g.move.power || '-'}
                            {g.attributes.power === 'up' && '↑'}
                            {g.attributes.power === 'down' && '↓'}
                        </div>

                        <div className={`col-span-1 flex items-center justify-center rounded-lg border font-bold ${g.attributes.accuracy === 'correct' ? 'bg-green-500 border-green-400' : 'bg-red-500 border-red-400'}`}>
                            {g.move.accuracy || '-'}
                            {g.attributes.accuracy === 'up' && '↑'}
                            {g.attributes.accuracy === 'down' && '↓'}
                        </div>

                        <div className={`col-span-1 flex items-center justify-center rounded-lg border font-bold ${g.attributes.pp === 'correct' ? 'bg-green-500 border-green-400' : 'bg-red-500 border-red-400'}`}>
                            {g.move.pp}
                            {g.attributes.pp === 'up' && '↑'}

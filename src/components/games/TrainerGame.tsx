...
const TrainerGame: React.FC<Props> = ({ onClose, date, seed }) => {
  ...
  if (!target || teamDetails.length === 0)
    return <div className="text-center p-8">Loading TrainerDle...</div>;
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-6 w-full">
        ...
        <h2 className="text-2xl font-black uppercase tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-red-600 to-black">
          TrainerDle
        </h2>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          {target.team.map((_, index) => {
            ...
            return (
              <div
                key={index}
                className={`w-20 h-20 rounded-full border-2 flex items-center justify-center overflow-hidden transition-all ${isRevealed ? 'bg-white border-red-500' : 'bg-slate-800 border-slate-700'}`}
              >
                {isRevealed && pokemon ? (

...
const PokemonMultiSelect: React.FC<PokemonMultiSelectProps> = ({
  ...
  return (
    <div ref={containerRef} className="grid gap-3">
      ...
      <div className="flex gap-2">
        ...
        >
          ...
          {filteredOptions.length > 0 &&
            `(${Math.min(filteredOptions.length, maxSelections - value.length)})`}
        </button>
        <button
          type="button"
          onClick={clearAll}
      ...
      {isOpen && (
        ...
        >
          <div className="overflow-y-auto" style={{ maxHeight }}>
            {filteredOptions.length === 0 ? (
              <div className="px-4 py-8 text-center text-slate-400">No Pokemon found</div>
            ) : (
                ...
                {filteredOptions.map((option) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center justify-between gap-3 px-3 py-2.5 rounded-md text-left transition ${
                        isSelected
      ...
      {/* Helper Text */}
      <p
        className={`text-xs ${value.length >= maxSelections ? 'text-amber-400' : 'text-slate-400'}`}
      >
        {value.length === 0

...
const AchievementModal: React.FC<AchievementModalProps> = ({ onClose }) => {
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title="Achievements" size="md">
      {/* Progress Section */}
      <div
        className={`p-6 border-b ${theme === 'dark' ? 'border-white/10 bg-black/20' : 'border-slate-100 bg-slate-50'}`}
      >
        ...
        <div
          className={`h-4 w-full rounded-full overflow-hidden ${theme === 'dark' ? 'bg-slate-800' : 'bg-slate-200'}`}
        >
          <div
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h3
                      className={`font-bold text-lg leading-tight mb-1 ${isUnlocked ? (theme === 'dark' ? 'text-yellow-400' : 'text-yellow-700') : ''}`}
                    >
                      {achievement.title}
                    </h3>
                    <p
                      className={`text-sm leading-snug ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}
                    >

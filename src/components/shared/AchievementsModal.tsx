  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Progress Bar */}
        <div className={`p-6 rounded-xl mb-6 border ${theme === 'dark' ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
            ...
            <p className={`text-right text-xs mt-2 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>
    ...
    return (
        <div
            className={`
                ...
                ${isUnlocked
                    ? (theme === 'dark' ? 'bg-gradient-to-br from-slate-800 to-slate-900 border-yellow-500/50 shadow-lg shadow-yellow-900/10' : 'bg-white border-yellow-400 shadow-md')
                    : (theme === 'dark' ? 'bg-slate-800/50 border-white/5 opacity-70' : 'bg-slate-50 border-slate-200 opacity-60 grayscale')
                }
            `}
        >
            <div className="flex items-start gap-4">
                <div
                    className={`
                        ...
                        ${isUnlocked
                            ? (theme === 'dark' ? 'bg-slate-800 border-2 border-yellow-500' : 'bg-yellow-50 border-2 border-yellow-400')
                ...
                <div className="flex-1">
                    <div className="flex justify-between items-start">
                        <h3 className={`font-bold text-lg ${isUnlocked ? (theme === 'dark' ? 'text-yellow-400' : 'text-yellow-700') : ''}`}>
                            {achievement.title}
                        </h3>
                        {isUnlocked && (
                    ...
                    <p className={`text-sm mt-1 leading-tight ${theme === 'dark' ? 'text-slate-300' : 'text-slate-600'}`}>

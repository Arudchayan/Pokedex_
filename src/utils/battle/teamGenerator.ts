...
const mapStats = (stats: any[]): StatSet => {
  return {
    ...
    def:
      stats.find((s: any) => s.stat?.name === 'defense' || s.name === 'defense')?.base_stat || 50,
    spa:
      stats.find((s: any) => s.stat?.name === 'special-attack' || s.name === 'special-attack')
        ?.base_stat || 50,
    spd:
      stats.find((s: any) => s.stat?.name === 'special-defense' || s.name === 'special-defense')
        ?.base_stat || 50,
  ...
  try {
    ...
    while (team.length < count && attempts < MAX_RETRIES) {
      ...
      try {
        ...
        const validPool = allMoves.filter(
          (m) => learnableNames.includes(m.name) && (m.power || m.accuracy)
        );
